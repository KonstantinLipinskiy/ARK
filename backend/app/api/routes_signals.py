...
from app.models import Signal  # Pydantic модель
from app.db.schemas import SignalORM  # SQLAlchemy модель
...
def create_signal(signal: Signal, db: Session = get_db()):
...
def delete_signal(signal_id: int, db: Session = get_db()):
	signal = db.query(SignalORM).filter(SignalORM.id == signal_id).first()
	if not signal:
		raise HTTPException(status_code=404, detail="Signal not found")
	db.delete(signal)
