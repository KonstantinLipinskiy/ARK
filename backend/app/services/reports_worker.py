...
class ReportsWorker:
	...
	async def handle_message(self, message):
		...
		try:
			...
			export_format = body.get("export_format", "text")  # text, pdf, html

			logger.info(f"📥 Получен запрос на отчёт (user_id={user_id}, format={export_format})")
			REPORT_REQUESTS_TOTAL.inc()

			# Генерация отчёта
			report_text = self.reports_service.generate_rag_report(trades, filters=filters)
			...
			avg_time = (self.reports_service.total_time + duration) / (self.reports_service.requests_count + 1)
			REPORT_AVG_RESPONSE_TIME.set(avg_time)
