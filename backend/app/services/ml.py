...
class MLService:
    ...
    def prepare_data(self, trades: list[dict]) -> pd.DataFrame:
        ...
        if "close" in df.columns:
            ...
            # 🔹 добавлено: упрощённое значение Bollinger
        ...
        # 🔹 добавлено: Stochastic Oscillator
        if {"high", "low", "close"} <= set(df.columns):
        ...
        # 🔹 добавлено: Ichimoku (упрощённый вариант)
        if "close" in df.columns:
        ...
        if "volume" in df.columns:
            ...
            # 🔹 добавлено: сохраняем текущее значение объёма
            df["volume"] = df["volume"]
    ...
    def train(self, df: pd.DataFrame, model_type: str = "sklearn") -> dict:
        ...
        if model_type == "sklearn":
            ...
            # 🔹 пока оставляем базовые признаки
    ...
    def predict_signal(self, features: dict) -> float:
        ...
        # 🔹 теперь features должен совпадать с расширенным набором
    ...
    def save_signal_embedding(self, features: dict, signal_id: int):
        try:
            ...
            # 🔹 формируем вектор из признаков для вставки в VectorDB
