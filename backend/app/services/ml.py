...
class MLService:
	...
	def prepare_data(self, trades: list[dict]) -> pd.DataFrame:
		...
		if "news" in df.columns:
			df = df[df["news"].apply(lambda x: isinstance(x, str) and len(x.strip()) > 20)]
		...
		try:
			...
			for _, row in df.iterrows():
					text = row.get("news", "")
					if text and self.sentiment_pipeline:
	...
	def load_model(self, path: str, model_type: str):
		...
		if model_type == "sklearn":
			...
			# 🔹 базовая архитектура для PyTorch (3 признака: ema, rsi, macd)
			model = nn.Sequential(
					nn.Linear(3, 16),
					nn.ReLU(),
					nn.Linear(16, 2)
	...
	def save_signal_embedding(self, features: dict, signal_id: int):
		...
		🔹 Доработки:
		- Используем гибкий размер вектора из settings
		- Нормализуем признаки до фиксированного размера
		- Проверяем дубликаты и отбрасываем мусорные данные
		- Проверяем дисперсию и валидность payload
		"""
		try:
			self.vector_db.use_collection("signals")
			values = [float(v) for v in features.values()]

			# 🔹 фильтрация: пустые или все нули
			if len(values) < 3 or np.allclose(values, 0):
					logger.info(f"Сигнал {signal_id} некорректный (слишком мало признаков или все нули)")
					return {"status": "skipped", "reason": "invalid features"}

			# 🔹 проверка дисперсии
			if np.var(values) < 1e-6:
					logger.info(f"Сигнал {signal_id} имеет слишком низкую дисперсию — не сохраняем")
					return {"status": "skipped", "reason": "low variance"}

			# 🔹 нормализация: повторяем признаки до нужного размера
			...
			if existing and not isinstance(existing, dict) and len(existing) > 0:
					logger.info(f"Сигнал {signal_id} уже существует в Qdrant — не сохраняем")
